...
@section('main')
    ...
    <form class= "bg-[#eff6ff] prose md:prose-lg lg:prose-xl rounded-b-lg  outline outline-offset-2 outline-cyan-500  mt-4 mb-4 overflow-y-auto w-4/6 p-3 mx-auto flex flex-col items-center justify-center" action="{{route('alumnos.store')}}" method="post">
        @csrf
        ...
        <input class="inline-block rounded-md " type="text" name="address" id="" value="{{old('phone')}}">
        <label class="block underline decoration-sky-800 underline decoration-double pb-1">Email</label>
        <input class="inline-block rounded-md " type="text" name="DNI" id="" value="{{old('email'}}">
        <label class="block underline decoration-sky-800 underline decoration-double pb-1">Empresa de prácticas</label>
        <input class="inline-block rounded-md " type="text" name="DNI" id="" value="{{old('empresa_id')}}">
